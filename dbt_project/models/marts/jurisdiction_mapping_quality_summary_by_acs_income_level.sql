...
/*
Primary-website mapping rates by **ACS median household income bucket**
(``jurisdiction_acs.income_level`` from B19013), joined in ``jurisdiction_mapping_analysis``.
Rows without ACS income (NULL) are excluded.
*/
...
WHERE acs_income_level IS NOT NULL
GROUP BY jurisdiction_type, acs_income_level
