...
1. Picks **county GEOIDs** from local meetings cache (`failed` / `shallow` / `missing` for AL) and optional `--geoids`.
2. Loads **`intermediate.int_jurisdictions`** names from Postgres.
3. Runs **DuckDuckGo** (`duckduckgo-search`) for an “official county government website” style query.
4. Prefers **`.gov` / `.org`** results, drops obvious junk hosts (Wikipedia, Facebook, …), scores hosts that contain a **slug** from the county name.
5. Validates with **`httpx` HEAD** (GET fallback if HEAD is `405` / `501`).
6. Optional **`--fetch-contact-hint`**: GET homepage and pull **title + first mailto** with BeautifulSoup (audit only).
...
- **CSV** (default): `data/cache/enrichment/jurisdiction_website_enrichment_{state}_{timestamp}.csv`  
  Columns include `manual_review`, `validation_detail`, `youtube_*`, and `notes` for a **manual review queue** when automation did not find a confident `.gov`/`.org` or HTTP check failed.

- **`--apply-seed`**: appends high-confidence rows (`manual_review=false`, HTTP 2xx/3xx) to  
  `dbt_project/seeds/jurisdiction_website_url_overrides.csv`.

- **`--skip-existing-seed`**: do not propose counties already listed in that seed.
