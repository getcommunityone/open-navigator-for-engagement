...
RUN npm ci
COPY frontend/ ./
# Build with base path for serving under /app
RUN npm run build

FROM python:3.11-slim

# Install system dependencies and nginx for reverse proxy
...
COPY --from=frontend-builder /build/dist /app/static/frontend
